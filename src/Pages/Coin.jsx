...
function Coin() {
  ...
  const handleClick = (e) => {
    ...
    if (input[buttonName] === 0) {
      ...
      axios.post(
        `${process.env.REACT_APP_BASE_URL}/coins/${data.usertype}/${buttonName}`,
        body,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/coins/${usertype}/balance`, {
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <Layout>
      <div id="Coins-container">
        <div id="Coins-portal">
          <h2 className="Coins-form-heading">
            Coins portal <i className="fa-solid fa-coins coin-icon"></i>{" "}
          </h2>
          ...
          <input
            id="buyInput"
            className="Coins-form-input"
            name="buy"
            type="number"
            onChange={onChangehandle}
            placeholder={0}
          ></input>
          <button className="btn btn-primary" name="buy" onClick={handleClick}>
            Buy
          </button>
          <br />
          <input
            ...
            placeholder={0}
          ></input>
          <button className="btn btn-primary" name="sell" onClick={handleClick}>
            Sell
          </button>
        </div>
      </div>
